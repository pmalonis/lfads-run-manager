classdef Run < LFADS.Run
    methods
        ...
        function out = generateCountsForDataset(r, dataset, mode, varargin) %#ok<INUSL,INUSD>
            ...
            dt = 0.001; % 1ms bin size
            data = dataset.loadData();
            out = RTP.read_data(data, dt);
            %if external input dimension parameter is 0, delete external
            %input
            if r.params.c_ext_input_dim == 0
                out.externalInputs = zeros(size(out.externalInputs,1),0, size(out.externalInputs,3));
            end
        end
    end
end
